...
async function fetchWithRetry(url, options, retries) {
  ...
  return new Promise((resolve, reject) => {
    operation.attempt(async (attemptNum) => {
      const fetch = await import('node-fetch');
      try {
        const response = await fetch.default(url, options);
        if (!response.ok) {
          throw new Error(`Received ${response.status} status code from ${url}`);
        }
        const result = await response.json();
        resolve(result);
      } catch (error) {
        if (operation.retry(error)) {
          console.error(`Attempt ${attemptNum} failed with error: ${error}. Retrying...`);
          return;
        }
  ...
  if (fileType === 'application/zip' || fileType === 'application/x-zip-compressed') {
    ...
    if (containsCppFiles) {
      ...
      pythonProcess.on('close', (code) => {
        ...
        console.log("found observations.json file")
...
app.post('/api/link', async(req, res) => {
  ...
  try {
    ...
    // Make a request to the GitHub API to get the download URL for the repository
    // const response = await fetchWithRetry(apiUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } }, 3);
    // if (!response) {
    //   return res.status(400).json({
    //     message: 'Error fetching zip file from GitHub',
    //   });
